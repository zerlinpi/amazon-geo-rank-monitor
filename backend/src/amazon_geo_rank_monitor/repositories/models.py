...
class RankAlertRuleRow(Base):
    __tablename__ = "rank_alert_rules"

    id: Mapped[str] = mapped_column(String(36), primary_key=True)
    owner_id: Mapped[str] = mapped_column(
        ForeignKey("tenants.id", ondelete="CASCADE"), index=True, nullable=False
    )
    monitor_target_id: Mapped[str | None] = mapped_column(String(36), index=True)
    name: Mapped[str] = mapped_column(String(200), nullable=False)
...
class RankAlertEventRow(Base):
    __tablename__ = "rank_alert_events"
    __table_args__ = (
        UniqueConstraint("rule_id", "fingerprint", name="uq_rank_alert_fingerprint"),
    )

    id: Mapped[str] = mapped_column(String(36), primary_key=True)
    owner_id: Mapped[str] = mapped_column(String(36), index=True, nullable=False)
    rule_id: Mapped[str] = mapped_column(
        ForeignKey("rank_alert_rules.id", ondelete="CASCADE"), index=True, nullable=False
    )
    monitor_target_id: Mapped[str] = mapped_column(String(36), index=True, nullable=False)
    run_id: Mapped[str] = mapped_column(String(36), index=True, nullable=False)
    asin: Mapped[str] = mapped_column(String(32), nullable=False)
    geo_profile_id: Mapped[str | None] = mapped_column(String(128))
    event_type: Mapped[str] = mapped_column(String(32), nullable=False)
    fingerprint: Mapped[str] = mapped_column(String(128), nullable=False)
    previous_value: Mapped[Decimal | None] = mapped_column(Numeric(12, 2))
...
class RankAlertDeliveryRow(Base):
    __tablename__ = "rank_alert_deliveries"

    id: Mapped[str] = mapped_column(String(36), primary_key=True)
    event_id: Mapped[str] = mapped_column(
        ForeignKey("rank_alert_events.id", ondelete="CASCADE"), index=True, nullable=False
    )
